...
CREATE TABLE IF NOT EXISTS products (
    ...
    ram_gb INT AS (JSON_EXTRACT(specs, '$.ram')) STORED,
    storage_gb INT AS (JSON_EXTRACT(specs, '$.storage')) STORED,
    gpu VARCHAR(100) AS (JSON_UNQUOTE(JSON_EXTRACT(specs, '$.gpu'))) STORED,
    screen_size DECIMAL(3,1) AS (JSON_EXTRACT(specs, '$.screen_size')) STORED,
    battery_mah INT AS (JSON_EXTRACT(specs, '$.battery_mah')) STORED,
...
-- Trigger for JSON normalization
DELIMITER $$
